...
const ProfileProperties = (props)=> {
    ...
    const [bedroom, setBedroom]  = React.useState(allProperties[index].details.bedroom);
    const [bathroom, setBathroom]  = React.useState(allProperties[index].details.bathroom);
    const [carPark, setCarPark]  = React.useState(allProperties[index].details.carPark);

    const [description, setDescription]  = React.useState(allProperties[index].propertyDescription);

    const [street, setStreet]  = React.useState(allProperties[index].address.street);
    const [suburb, setSuburb]  = React.useState(allProperties[index].address.suburb);
    const [state, setState]  = React.useState(allProperties[index].address.state);
    const [postcode, setPostcode]  = React.useState(allProperties[index].address.postcode);

    const [propertyType,setPropertyType] = useState(allProperties[index].propertyType);
    ...
    const updateInformation = () => {
		...
		axios.post("http://localhost:8080/api/v1/properties/updateProperty", allProperties[index])
			.then((res) => {
                        ...
                        <Grid container>
                                    ...
                                    <Grid xs={6} paddingInline={2}>
                                        ...
                                        <div className="horizontal">
                                            <div className="heading">
                                                <h3 > $ {allProperties[index].rentalPrice}  </h3>
                                            </div>
                                            <div className="heading">
                                                <h3 > RENT DATE:  </h3>
                                            </div>
                                            ...
                                            <div className="horizontal">
                                                <div>
                                                    ...
                                                    { <TextField fullWidth className="formfield" color="secondary" value={bedroom} onChange={(event) => setBedroom(event.target.value)} /> }
                                                </div>
                                                <div>
                                                    <p className="formfield">Bathrooms</p>
                                                    { <TextField fullWidth className="formfield" color="secondary" value={bathroom} onChange={(event) => setBathroom(event.target.value)}/> }
                                                </div>
                                                <div>
                                                    <p className="formfield">Car</p>
                                                    { <TextField fullWidth className="formfield" color="secondary" value={carPark} onChange={(event) => setCarPark(event.target.value)}/> }
                                                </div>
                                            </div>
                                        ...
                                        <Grid xs={12} paddingInline={2}>
                                            ...
                                            <TextField fullWidth className="formfield" color="secondary" value={description} onChange={(event) => setDescription(event.target.value)}/>
                                        </Grid>

                                        <Grid xs={6} paddingInline={2}>
                                            <p className="fieldLabel">Street</p>
                                            <TextField fullWidth className="formfield" color="secondary" value={street} onChange={(event) => setStreet(event.target.value)}/>
                                        </Grid>
                                        <Grid xs={6} paddingInline={2}>
